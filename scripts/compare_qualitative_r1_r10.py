#!/usr/bin/env python3
"""Compare qualitative text-to-image R1-R10 retrieval results for two RDE checkpoints."""
...
MODEL_FAMILY = "RDE"
RETRIEVAL_MODES = ("auto", "bge", "tse", "bge+tse")
SORT_MODES = (
    "best_r1_green_gap",
    "strict_showcase",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate two RDE checkpoints on the full test split, rank every caption "
    ...
    parser.add_argument(
        ...
        default="auto",
        help="RDE official retrieval branch. auto resolves to bge+tse.",
    )
    ...
    parser.add_argument("--select_ratio", type=float, default=None, help="Override RDE TSE token selection ratio.")
    parser.add_argument("--loss_names", default=None, help="Override RDE loss/task string used by build_model.")
...
def build_eval_args(cli_args: argparse.Namespace) -> SimpleNamespace:
    from utils.ablation import finalize_ablation_args
    ...
    if cli_args.select_ratio is not None:
        cfg["select_ratio"] = float(cli_args.select_ratio)
    if cli_args.loss_names is not None:
        cfg["loss_names"] = str(cli_args.loss_names)
    if cli_args.tau is not None:
        cfg["tau"] = float(cli_args.tau)
    if cli_args.margin is not None:
        cfg["margin"] = float(cli_args.margin)
    ...
    cfg.setdefault("select_ratio", 0.3)
    cfg.setdefault("tau", 0.015)
    cfg.setdefault("margin", 0.1)
    ...
    return finalize_ablation_args(SimpleNamespace(**cfg))
...
def resolve_retrieval_mode(mode: str) -> str:
    if mode == "auto":
        return "bge+tse"
    if mode not in {"bge", "tse", "bge+tse"}:
...
def embedding_cache_metadata(
    ...
    return {
        ...
        "select_ratio": float(getattr(model_args, "select_ratio", 0.3)),
        "loss_names": str(getattr(model_args, "loss_names", "TAL")),
        "tau": float(getattr(model_args, "tau", 0.015)),
        "margin": float(getattr(model_args, "margin", 0.1)),
        "prototype": bool(getattr(model_args, "prototype", False)),
        "use_loss_id": bool(getattr(model_args, "use_loss_id", False)),
        "no_pbt": bool(getattr(model_args, "no_pbt", False)),
        "no_ira": bool(getattr(model_args, "no_ira", False)),
        "no_iopm": bool(getattr(model_args, "no_iopm", False)),
    }
...
def combine_rde_features(
    bge_text: Optional[torch.Tensor],
    bge_image: Optional[torch.Tensor],
    tse_text: Optional[torch.Tensor],
    tse_image: Optional[torch.Tensor],
    retrieval_mode: str,
) -> Tuple[torch.Tensor, torch.Tensor]:
    if retrieval_mode == "bge":
        if bge_text is None or bge_image is None:
            raise RuntimeError("BGE features were not extracted.")
        return bge_text, bge_image
    if retrieval_mode == "tse":
        if tse_text is None or tse_image is None:
            raise RuntimeError("TSE features were not extracted.")
        return tse_text, tse_image
    if retrieval_mode == "bge+tse":
        if bge_text is None or bge_image is None or tse_text is None or tse_image is None:
            raise RuntimeError("Both BGE and TSE features are required for bge+tse.")
        scale = math.sqrt(0.5)
        return torch.cat([scale * bge_text, scale * tse_text], dim=1), torch.cat(
            [scale * bge_image, scale * tse_image], dim=1
        )
    raise ValueError(f"Unsupported RDE retrieval mode: {retrieval_mode!r}")
...
def load_or_extract_embeddings(
    ...
    need_bge = retrieval_mode in {"bge", "bge+tse"}
    need_tse = retrieval_mode in {"tse", "bge+tse"}
    query_pids: Optional[torch.Tensor] = None
    gallery_pids: Optional[torch.Tensor] = None
    bge_text: Optional[torch.Tensor] = None
    bge_image: Optional[torch.Tensor] = None
    tse_text: Optional[torch.Tensor] = None
    tse_image: Optional[torch.Tensor] = None

    if need_bge:
        bge_text, query_pids = extract_text_features_from_encoder(
            model, test_txt_loader, device, "encode_text", f"{label} text BGE"
        )
        bge_image, gallery_pids = extract_image_features_from_encoder(
            model, test_img_loader, device, "encode_image", f"{label} image BGE"
        )

    if need_tse:
        tse_text, tse_query_pids = extract_text_features_from_encoder(
            model, test_txt_loader, device, "encode_text_tse", f"{label} text TSE"
        )
        tse_image, tse_gallery_pids = extract_image_features_from_encoder(
            model, test_img_loader, device, "encode_image_tse", f"{label} image TSE"
        )
        if query_pids is None:
            query_pids = tse_query_pids
        else:
            ensure_same_tensor(query_pids, tse_query_pids, f"{label} query pid")
        if gallery_pids is None:
            gallery_pids = tse_gallery_pids
        else:
            ensure_same_tensor(gallery_pids, tse_gallery_pids, f"{label} gallery pid")

    if query_pids is None or gallery_pids is None:
        raise RuntimeError(f"{label}: no query/gallery pids were extracted.")

    text_features, image_features = combine_rde_features(
        bge_text, bge_image, tse_text, tse_image, retrieval_mode
    )
...
def retrieval_formula(retrieval_mode: str) -> str:
    if retrieval_mode == "bge":
        return "s_BGE"
    if retrieval_mode == "tse":
        return "s_TSE"
    if retrieval_mode == "bge+tse":
        return "0.5 * s_BGE + 0.5 * s_TSE"
    raise ValueError(f"Unsupported RDE retrieval mode: {retrieval_mode!r}")


def rde_bge_weight(retrieval_mode: str) -> float:
    if retrieval_mode == "bge":
        return 1.0
    if retrieval_mode == "tse":
        return 0.0
...
def build_summary(
    ...
    summary = {
        ...
        "best_name": best_name,
        "retrieval_mode": retrieval_mode,
        "global_weight": rde_bge_weight(retrieval_mode),
        "bge_weight": rde_bge_weight(retrieval_mode),
        "retrieval_formula": retrieval_formula(retrieval_mode),
...
def main() -> None:
    ...
    print(f"[Retrieval] {MODEL_FAMILY} mode={retrieval_mode} formula={retrieval_formula(retrieval_mode)}")
