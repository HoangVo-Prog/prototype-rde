...
def _prototype_requested(args):
    return prototype_requested(args)
...
def do_train(start_epoch, args, model, train_loader, evaluator, optimizer,
    ...
    if _should_run_initial_eval(start_epoch, eval_after_epoch):
        initial_model = model.module if getattr(args, "distributed", False) else model
        initial_eval = evaluator.eval(initial_model.eval(), return_metrics=(get_rank() == 0))
